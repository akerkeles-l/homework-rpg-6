...
public class TournamentEngine {
    ...
    public TournamentResult runTournament() {
        ...
        while (round < maxRounds && hero.isAlive() && opponent.isAlive()) {
            ...
            result.addLine("[Round " + round + "] Hero HP: " + hero.getHealth() + " | Opponent HP: " + opponent.getHealth());
            
            actionQueue.enqueue(new AttackCommand(opponent, hero.getAttackPower()));
            
            if (hero.getHealPotions() > 0 && hero.getHealth() < hero.getMaxHealth() * 0.6) {
            ...
            if (opponent.isAlive()) {
                result.addLine("  Opponent attacks for " + opponent.getAttackPower() + " damage!");
                defenseChain.handle(opponent.getAttackPower(), hero);
            }
            
            result.addLine("  After combat - Hero HP: " + hero.getHealth() + " | Opponent HP: " + opponent.getHealth());
        }
